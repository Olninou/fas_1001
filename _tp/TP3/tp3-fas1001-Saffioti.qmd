---
title: "Analyse de l'impact du genre sur la saillance des thématiques politiques féminines dans le discours des politicien(ne)s"
...
Fréchet, Nadjim. 2024. *Base de données portant sur le débat électoral entre Emmanuel Macron et Marine Le Pen de 2017*.
...
debat_macron_lepen <-debat_macron_lepen %>% select(speaker, text_clean) %>% filter(speaker %in% c("Le Pen", "Macron"))


## Appliquer le dictionnaire : 

debat_macron_lepen_resultats <- run_dictionary(data = debat_macron_lepen, text = text_clean, dictionary = Dictionnaire_combiné) %>% bind_cols(debat_macron_lepen) %>% select(-c(doc_id))
